...
function [ht,oos_predict,b]=my_HAR(date_ser,rv,mdl_type)
if nargin<2
...
%% HAR regression
if mdl_type==1
    X=[ones(numel(RV_w)-1,1),RV_Day(m_length:end-1),RV_w(1:end-1)',RV_m(1:end-1)'];
    Y=RV_Day(m_length+1:end);
    b = (X'*X)\(X'*Y);
    Forecasted_Y = X*b;
    oos_predict=[1,RV_Day(end),RV_w(end),RV_m(end)]*b;
    ht=[nan(m_length,1);Forecasted_Y];
else
    
    X=[ones(numel(RV_w)-1,1),log(RV_Day(m_length:end-1)),log(RV_w(1:end-1)'),log(RV_m(1:end-1)')];
    Y=log(RV_Day(m_length+1:end));
    b = (X'*X)\(X'*Y);
    Forecasted_logY = X*b;
    Forecasted_Y=exp(Forecasted_logY);
    logoos_predict=[1,log(RV_Day(end)),log(RV_w(end)),log(RV_m(end))]*b;
