%% This script generates the results in Tables 4, 7 and 8 in the manuscript
...
cases_list={'High Stress','Low Stress'};
freq=5; %mins
...
perf_table=table();
iter=0;
for case_idx=1:numel(cases_list)
    current_case=cases_list{case_idx};
    disp(['Calculations started for ',current_case,' profile ...']);
    for t=1:numel(main_ticker)
        ...
        if case_idx==1
            target_idx=find(and(tbl0.FSIVol>=0,year(tbl0.Date)>=2014));
            oos_idx_range=target_idx-TF1SMP+1;
        elseif case_idx==2
            target_idx=find(and(tbl0.FSIVol<0,year(tbl0.Date)>=2014));
            oos_idx_range=target_idx-TF1SMP+1;
        end
        ...
        for l=1:numel(loss_b_range)
            ...
            perf_table{iter,'Risk_Profile'}={current_case};
