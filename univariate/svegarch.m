function [parameters, LL, ht, VCVrobust, VCV, scores, diagnostics] = svegarch(data, p, o, q, error_type, MuCondVar, startingvals, options)
...
%   MuCondVar     - Conditional variance average.
...
%
%  See also svegarch_likelihood, EGARCH_CORE, EGARCH_PARAMETER_CHECK, EGARCH_STARTING_VALUES, 
...
% Modified by Arman Hassanniakalager
...
switch nargin
    case 4
        [p,o,q,error_type,startingvals,options]=svegarch_parameter_check(data, p, o, q);
    case 5
        [p,o,q,error_type,startingvals,options]=svegarch_parameter_check(data, p, o, q, error_type);
    case 6
        [p,o,q,error_type,startingvals,options]=svegarch_parameter_check(data, p, o, q, error_type, MuCondVar);
    case 7
        [p,o,q,error_type,startingvals,options]=svegarch_parameter_check(data, p, o, q, error_type, MuCondVar, startingvals);
    case 8
        [p,o,q,error_type,startingvals,options]=svegarch_parameter_check(data, p, o, q, error_type, MuCondVar, startingvals, options);
    otherwise
        error('Number of inputs must be between 4 and 9');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input Checking
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
LL0=svegarch_likelihood(startingvals,data_augmented,p,o,q,error_type,MuCondVar,back_cast,T,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Starting values
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[parameters,LL,exitflag,output]=fmincon('svegarch_likelihood',startingvals,[],[],[],[],LB,UB,'egarch_nlcon',options,data_augmented,p,o,q,error_type, back_cast,T,1);
...
if  exitflag<=0 && LL<LL0
    ...
    [parameters,LL,exitflag,output]=fmincon('svegarch_likelihood',parameters,[],[],[],[],[],[],'egarch_nlcon',options,data_augmented,p,o,q,error_type,back_cast,T,1);
end
...
if startingflag==0 && exitflag<=0
    ...
    while exitflag<=0
        ...
        [parameters,LL,exitflag,output]=fmincon('svegarch_likelihood',startingvals,[],[],[],[],[],[],'egarch_nlcon',options,data_augmented,p,o,q,error_type ,back_cast,T,1);
        if  exitflag<=0 && LL<LL0
...
if nargout>1
    [LL, likelihoods, ht]=svegarch_likelihood(parameters,data_augmented,p,o,q,error_type, MuCondVar,back_cast,T);
    LL=-LL;
end
...
if nargout>3
    nw=0; %No newey west
    [VCVrobust,A,B,scores,hess]=robustvcv('svegarch_likelihood',parameters,nw,data_augmented,p,o,q,error_type,back_cast,T);
    VCV=hess^(-1)/(T-m);
end
