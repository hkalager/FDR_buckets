function [LL, LLS, ht] = svegarch_likelihood(parameters, data, p, o, q, error_type, MuCondVar, back_cast, T, estim_flag)
...
if nargin==10 && length(estim_flag==1) && all(estim_flag==1) %If for estimation, transform the parameters
    [parameters]=egarch_itransform(parameters,p,o,q,error_type);
elseif nargin==9 || (length(estim_flag)==1 && all(estim_flag==0))
    %Nothing to do
else
...
ht=svegarch_core(data,parameters,MuCondVar,back_cast,upper,p,o,q,m,T);
