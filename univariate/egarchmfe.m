function [parameters, LL, ht, VCVrobust, VCV, scores, diagnostics] = egarchmfe(data, p, o, q, error_type, startingvals, options)
...
LB=ones(size(startingvals))*-1e5;
%Need to keep alpha and gamma from making the process crazy
LB(2:p+o+1)=-3;
UB=ones(size(startingvals))*1e5;
UB(2:p+o+1)=3;
